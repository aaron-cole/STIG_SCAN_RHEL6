...
#STIG Identification
GrpID="V-218063"
GrpTitle="SRG-OS-000228"
RuleID="SV-218063r505923_rule"
...
if rpm -q GConf2 >> $Results; then
 if [ "$(gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --get /apps/gdm/simple-greeter/banner_message_text 2>>/dev/null)" == "$banner" ]; then 
  gconftool-2 --direct --config-source xml:readwrite:/etc/gconf/gconf.xml.mandatory --get /apps/gdm/simple-greeter/banner_message_text 2>>/dev/null >> $Results
  echo "Pass" >> $Results
 else 
